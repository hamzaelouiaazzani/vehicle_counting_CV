...
def main(vid_strides):
    ...
    for vid_stride in vid_strides:
        ...
        path = os.path.join(os.getcwd(), "experiments", "Cropped_Videos_Experiments" , "approach_1" , "Systematic_Line_Counting" , f"vid_stride_{vid_stride}_approach1.json")


        
        with open(path, 'w') as outfile:
            json.dump(overall_results, outfile)
        print(f"File vid_stride_{vid_stride}_approach1.json is saved!")
        
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run counting experiment for specified strides!")
